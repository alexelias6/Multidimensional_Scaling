...
```
In order to incorporate information about rear seat room and luggage capacity, vans and two-seater vehicles are excluded from the dataset. These are the only types of vehicles missing these values.
```{r}
Cars93 = Cars93[!is.na(Rear.seat.room) & !is.na(Luggage.room)]
```
Next, standardize the variables using the scale() function to change all measures to the same scale.
```{r}
Cars93_cont_scaled = Cars93[,.(Min.Price, Price, Max.Price,
                      ...
                      scale()
```
Then calculate the dissimilarity matrix of your choosing. The code below computes four different dissimilarity matrices. Any of these four measures of distance can be used for MSD. 
```{r}
...
```
Lastly, perform MDS using the torgerson() function from the smacof package. This function performs classical MDS and takes the dissimilarity matrix as its argument. It may be useful to note that torgeson() has an optional argument to specify the dimensions of the resulting coordinates. We chose the default of 2 dimensions because it is easiest to visualize. However, using higher dimensions can improve fit. The code below performs MDS using Euclidian distance.
```{r}
# Euclidean Distance
...
```
The code below performs MDS using Manhattan distance.
```{r}
# Manhattan Distance
...
ggplot(df_manhattan, aes(x=Z1,y=Z2),main='Main') + 
  ...
  geom_text(aes(label=Model), hjust=-0.2, size=3)
```
The code below performs MDS using Maximum distance.
```{r}
# Maximum Distance 
...
ggplot(df_maximum, aes(x=Z1,y=Z2),main='Main') + 
  ...
  geom_text(aes(label=Model), hjust=-0.2, size=3)
```
The code below performs MDS using correlation.
```{r}
# Correlation
