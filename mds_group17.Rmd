...
```
The only continuous variables with missing values are Rear.seat.room and Luggage.room. We drop the observations containing these
missing values.
```{r}
Cars93 = Cars93[!is.na(Rear.seat.room) & !is.na(Luggage.room)]
```
Next, we standardize the variables using the scale() function to give all variables equal weighting.
```{r}
...
ggplot(df_maximum, aes(x=Z1,y=Z2),main='Main') + 
  ...
  geom_text(aes(label=Model), hjust=-0.2, size=3)
```
The code below performs MDS using the correlation-based dissimilarity measure.
```{r}
# Correlation
